...
output "backend_config_hcl" {
  ...
  value = [for k in var.github_environments : <<-EOT
    # Generated by iac/bootstrap. Paste into iac/env/${k}.backend.hcl and set `key`.
...
output "github_actions_setup" {
  ...
  value       = <<-EOT
    ...
    (RG-scoped Owner, never subscription scope). Each UMI can contribute to
    its own tfstate container and currently has account-level Reader access,
    so workload write isolation does not imply cross-environment state secrecy.
