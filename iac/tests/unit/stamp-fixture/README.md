...
A near-verbatim copy of `iac/modules/stamp/` used **only** by
`iac/tests/unit/stamp.tftest.hcl`. Do not deploy this fixture to any
environment.

## Why this fixture exists

The real stamp module in `iac/modules/stamp/keyvault.tf` declares two
...
The directory mirrors `iac/modules/stamp/` 1:1 with two intentional
exceptions:
...
Whenever any `iac/modules/stamp/*.tf` file changes, update the matching
`iac/tests/unit/stamp-fixture/*.tf` file in the same commit.

Quick check:

```sh
diff -r iac/modules/stamp/ iac/tests/unit/stamp-fixture/
```
...
resources, delete this fixture and point `iac/tests/unit/stamp.tftest.hcl`
directly at `iac/modules/stamp/`.
