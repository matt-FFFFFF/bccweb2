...
run "module_plans_with_minimum_inputs" {
  ...
  assert {
    condition     = azapi_resource.rg.name == "rg-bccweb-unit" && azapi_resource.function_app.name == "func-bccweb-unit"
    error_message = "The stamp module should plan successfully and expose expected core resource names."
  }
}
