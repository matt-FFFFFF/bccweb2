...
1. Run `terraform -chdir=iac/service init -backend-config=../env/prod.backend.hcl` and `terraform -chdir=iac/service apply -var-file=../env/prod.tfvars`. This provisions the SWA (the ACS email domain itself is owned by the `iac/common` stack — apply that first), but does **not** by itself create the public CNAME unless `var.dns_zone_name` is set.
2. Read the operator-facing outputs:
   ```bash
terraform -chdir=iac/common output acs_email_domain_verification_records
terraform -chdir=iac/service output -raw production_hostname_target
terraform -chdir=iac/service output -raw production_dns_managed_by_terraform
   ```
...
**Terraform path:** `iac/service/modules/stamp/dns.tf` hard-codes `ttl = 3600`. To run the lower-TTL phase, manually `az network dns record-set cname update --ttl 300 ...` 24h before cutover, then re-run `terraform -chdir=iac/service apply -var-file=../env/prod.tfvars` after the stability window to let Terraform reassert 3600. Do **not** edit `ttl` in `dns.tf` during the cutover window — that would race with the operator's portal change.
...
For each record returned by `terraform -chdir=iac/common output acs_email_domain_verification_records`:
...
terraform -chdir=iac/service apply -var-file=../env/prod.tfvars
```
...
24h after stable traffic, run `terraform -chdir=iac/service apply -var-file=../env/prod.tfvars` again to let Terraform reset the TTL to 3600.
...
4. At cutover: change the target to the value returned by `terraform -chdir=iac/service output -raw production_hostname_target`. Keep TTL at 300s.
...
SWA_HOST="$(terraform -chdir=iac/service output -raw production_hostname_target)" \
