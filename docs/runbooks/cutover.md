...
1. **Verify Infrastructure State**: Run `terraform -chdir=iac/environment plan -var-file=../env/<env>.tfvars -var 'terraform_principal_type=User'` (local apply — drop the `-var` override when running via the `terraform.yml` CI workflow) to ensure the state is current. Ensure the plan shows zero changes and specifically verify that blob versioning is enabled, soft-delete retention is 7 days, and GRS replication is configured. The management lock must be present on the storage account to prevent accidental deletion during the cutover.
...
1. **ACS Email Domain**: Execute the DNS verification in the registrar using the lowercase operator records provided by `terraform -chdir=iac/environment output acs_dns_records_for_operator` (`domain_ownership`, `spf`, `dkim`, `dkim2`, and `dmarc`, each carrying Azure's `type`, `name`, and `value`). Publish DMARC with `p=none` for first cutover — tighten after one clean week. Confirm verification in the Azure Portal under the Communication Services resource.
2. **CNAME Update**: Derive the unconditional SWA default host from `terraform -chdir=iac/environment output -raw swa_url` (remove `https://` and the trailing slash). When both `production_hostname` and `dns_zone_name` are set, Terraform owns the CNAME via `iac/environment/modules/stamp/dns.tf`; follow the managed path in `docs/runbooks/dns-cutover.md`. Otherwise, update the registrar and use `production_hostname_target` as the target when provided.
