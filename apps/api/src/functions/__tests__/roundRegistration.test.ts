...
describe("round self-registration endpoints", () => {
  ...
  it("25 distinct pilots register concurrently exactly once", async () => {
    ...
    const registrations: Array<{ pilot: Pilot; userId: string; email: string }> = [];
    for (let index = 0; index < 25; index += 1) {
      const pilot = await makePilot({ firstName: `Pilot${index}`, lastName: "Concurrent", clubId });
    ...
    const filledSlots = saved?.teams[0].pilots.filter((slot) => slot.status === "Filled") ?? [];
    const pilotIds = registrations.map(({ pilot }) => pilot.id);
    expect(filledSlots.map((slot) => slot.pilotId).toSorted()).toEqual(pilotIds.toSorted());
...
function trackRoundLeaseAttempts(roundPath: string): () => number {
  const originalAcquireLease = BlobLeaseClient.prototype.acquireLease;
  let roundAttempts = 0;
  vi.spyOn(BlobLeaseClient.prototype, "acquireLease").mockImplementation(function (
