...
**Framework**: Vitest 4.1.2 (root devDep). Workspace at [vitest.workspace.ts](file:///Volumes/code/bccweb2/vitest.workspace.ts) covers
`packages/{scoring,types}` and `apps/{api,web}`.
