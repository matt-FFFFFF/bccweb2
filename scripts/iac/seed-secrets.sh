...
# The Application Insights connection string (T46) is generated by Terraform
# but never written to Terraform state in plaintext (it is a sensitive output
# only). This script reads the sensitive output and places it into Key Vault
# so the Function App can resolve it at runtime via the @Microsoft.KeyVault
# reference in functions.tf app_settings.

AI_SECRET="appinsights-connection-string"

if az keyvault secret show \
     --vault-name "$VAULT" \
     --name "$AI_SECRET" \
     --query "id" -o tsv 2>/dev/null | grep -q .; then
  echo "[$AI_SECRET] already exists — skipping (delete the secret version to rotate)"
else
  AI_CONN="$(terraform -chdir="$IAC_DIR" output -raw application_insights_connection_string 2>/dev/null || true)"
  if [[ -z "$AI_CONN" ]]; then
    echo "[$AI_SECRET] terraform output application_insights_connection_string empty — skipping" >&2
  else
    az keyvault secret set \
      --vault-name "$VAULT" \
      --name "$AI_SECRET" \
